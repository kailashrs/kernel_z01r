...
{
	uint8_t capacity=0;
	if(asus_util_fs_read_text_uint8(BATTERY_CAPACITY,&capacity) == 0)
	{
...
static ssize_t flash_led_store(struct file *dev, const char *buf, size_t count, loff_t *loff)
{
	...
	n = sscanf(kbuf, "%d %d %d", &mode, &on, &val);

	pr_info("node %d, command %d %d %d n=%d\n",*index, mode, on, val,n);
	...
	if(mode == 0)//torch
	{
		if(on == 1)
		{
			...
			if(!(n == 3 && val > 0))//user not define current
			{
				val = ATD_TORCH_CURRENT;
			}
