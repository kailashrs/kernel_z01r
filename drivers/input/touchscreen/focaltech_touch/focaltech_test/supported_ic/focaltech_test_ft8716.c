 ...
 * Return: none
 ***********************************************************************/
static void save_test_data(int *data,char *test_num, u8 tx_num, u8 rx_num, u8 item_count)
{
    int len = 0;
    int i = 0;
    int j = 0;
    ...
    len = sprintf(test_data.tmp_buffer, "%s, %d, %d, %d, %d, %d, ", \
                  test_num,test_data.test_item_code, tx_num, rx_num, test_data.start_line, item_count);
...
static bool compare_data(int *data, int min, int max, int vk_min, int vk_max, bool key_support,int testitem)
{
    int chx = 0;
    int chy = 0;
    u8 tx_num = 0;
...
static int get_cb(u16 saddr, int byte_num, u8 *cb_buf)
{
    ...
    for (i = 0; i < packet_num; i++) {
        offset = read_num * i;
        //addr = saddr + offset;janchen
        //addr_h = (addr >> 8) & 0x0F;janchen
        //addr_l = addr & 0x0F; janchen
        addr_h = (saddr+usTotalReturnNum) >>8;//Address offset high 8 bit
        addr_l  = (saddr+usTotalReturnNum)&0xff;//Address offset low 8 bit
        if ((i == (packet_num - 1)) && packet_remainder) {
            read_num = packet_remainder;
        }
        ...
        // ret = fts_test_i2c_read(&cb_addr, 1, cb_buf + offset, read_num); janchen
        ret = fts_test_i2c_read(&cb_addr, 1, cb_buf + usTotalReturnNum, read_num);
        if (ret) {
            FTS_TEST_SAVE_INFO("read cb fail");
            return ret;
        }
        usTotalReturnNum += read_num;
    }
...
int ft8716_rawdata_test(bool *test_result)
{
    ...
    //////////////////////////////Save Test Data
    save_test_data(rawbuf,"RawData Test", tx_num + 1, rx_num, 1);
    //----------------------------------------------------------Return Result
    FTS_TEST_SAVE_INFO("\n==========RawData Test is %s. \n\n", (temp_result ? "OK" : "NG"));
...
int ft8716_cb_test(bool *test_result)
{
    ...
    //////////////////////////////Save Test Data
    //save_test_data(cb_buf,"CB Test", tx_num + 1, rx_num, 1);
    FTS_TEST_SAVE_INFO("\n========== CB Test is %s. \n\n", (temp_result ? "OK" : "NG"));
...
int ft8716_short_test(bool *test_result)
{
    ...
    } else if (test_data.screen_param.selected_ic == IC_FT8716U || test_data.screen_param.selected_ic == IC_FT8613|| test_data.screen_param.selected_ic == IC_FT8716F) {
    ...
    max = 100000000;
    temp_result = compare_data(adc_data, min, max, min_vk, max, key_support,0);

    save_test_data(adc_data,"Short Circuit Test", tx_num + 1, rx_num, 1);
...
int ft8716_open_test(bool *test_result)
{
    ...
    /* set driver mode */
    if (test_data.screen_param.selected_ic == IC_FT8716U || test_data.screen_param.selected_ic == IC_FT8613 || test_data.screen_param.selected_ic == IC_FT8716F) {
    ...
    cb_max = 256;
    temp_result = compare_data(cb_buf, cb_min, cb_max, 0, 0, false,0);

    save_test_data(cb_buf,"Open Test", tx_num + 1, rx_num, 1);
...
int ft8716_lcd_noise_test(bool *test_result)
{
    ...
    while(DataReady_Cnt <= 3){
			DataReady_Cnt++;
                     ret = write_reg( FACTORY_REG_LCD_NOISE_START, 0x01 );
                     printk("REG_LCD_NOISE_STAR_ReCode = %d, ", ret);
			if ( ret != ERROR_CODE_OK )
			{
                         FTS_TEST_SAVE_INFO("start lcd noise test fail");
                         goto LCDNOISE_TEST_ERR;
			}else{
				printk(" start lcd noise tes is Ok!\n");
			}
    }
    DataReady_Cnt = 0;
    ...
    temp_result = compare_data(diff_buf, 0, max, 0, max_vk, true,1);

    //save_test_data(diff_buf,"LCD Noise Test", tx_num + 1, rx_num, 1 );
...
bool start_test_ft8716(void)
{
    ...
    for (item_count = 0; item_count < test_data.test_num; item_count++) {
        ...
        if (CODE_FT8716_CB_TEST == item_code) {
             ...
             save_test_data(cb_buf,"CB Test", test_data.screen_param.tx_num + 1, test_data.screen_param.rx_num, 1 );
        }
		...
		for(i = 0 ; i < 5 ; i++){
		 ...
		 save_test_data(diff_buf,"LCD Noise Test", test_data.screen_param.tx_num + 1, test_data.screen_param.rx_num, 1 );
        }
